...
import { getModulesUserById, savePermissionsLocalStorage } from "../../../../redux/actions/Configuration/usersAction";
import { SettingOutlined, DashboardOutlined, BarChartOutlined, DollarCircleOutlined, UserOutlined, ProjectOutlined, CreditCardOutlined, CodepenCircleOutlined, } from "@ant-design/icons";
...
const NavMenu = () => {
	...
	const [colorStateMenu, setColorStateMenu] = useState(generalSetting?.menu_bar_color || colorMenu)
	const [colorStateIcon, setColorStateIcon] = useState(generalSetting?.icon_color || colorIcon)
	...
	useEffect(() => {
		setLoading(true)
		dispatch(getModulesUserById(userId, async (response) => {
	...
	function getIcon(type) {
		switch (type) {
			case "dashboard":
				return (<DashboardOutlined style={{ color: `${colorStateIcon}` }} />);
			case "setting":
				return (<SettingOutlined style={{ color: `${colorStateIcon}` }} />);
			case "project":
				return (<ProjectOutlined style={{ color: `${colorStateIcon}` }} />);
			case "user":
				return (<UserOutlined style={{ color: `${colorStateIcon}` }} />);
			case "creditCard":
				return (<CreditCardOutlined style={{ color: `${colorStateIcon}` }} />);
			case "dollarCircle":
				return (<DollarCircleOutlined style={{ color: `${colorStateIcon}` }} />);
			case "barChart":
				return (<BarChartOutlined style={{ color: `${colorStateIcon}` }} />);
			default:
				return (<CodepenCircleOutlined style={{ color: `${colorStateIcon}` }} />)
		}
	}
	const MenuItemWithIcon = ({ icon, title }) => (
		<div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', height:25, justifyContent: "space-evenly", color: `${colorStateIcon}`  }}>
			{icon}
			<br />
	...
	const listModules = modules?.map((module, index) => {
		if (module?.sub_modules?.length === 0) {
			...
			return (
				collapsed === false ?
					...
					<Menu.Item key={index} icon={getIcon(module.icon)}  >
						<Link to={route_front} style={{ color: `${colorStateIcon}`}}>{module.name}</Link>
					</Menu.Item> :
					<>
					<MenuItemWithIcon
							icon={<DashboardOutlined style={{ color: `${colorStateIcon}`,zIndex: 1  }} />}
							title="Dashboard"
						/>	
			...
			return (
				collapsed === false ?
				<SubMenu style={{ background: `${colorStateMenu}`, color: `${colorStateIcon}` }} key={index}
					icon={getIcon(module.icon)} title={module.name}>
					{sub_modules}
				</SubMenu>:
				<>
		
					
					<SubMenu style={{ color: `${colorStateIcon}`,position: "relative",zIndex: 1 }} key={index}
					 >
						{sub_modules}
					</SubMenu>

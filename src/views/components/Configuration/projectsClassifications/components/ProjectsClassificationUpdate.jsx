...
import { updateProjectsClassificationById, fetchAllProjectsClassification } from '../../../../../redux/actions/Configuration/ProjectsClassificationsAction';

const ProjectsClassificationUpdate = () => {

	const dispatch = useDispatch();
  const [form] = Form.useForm();
  const { projectsClassification } = useContext(ProjectsClassificationContext);
  const [name, setName] = useState((projectsClassification.name == null)?'':projectsClassification.name);
  const [state, setState] = useState(true);
  const [id, setId] = useState(projectsClassification.id);
  ...
  const showModal = () => {
    ...
    updateStateProjectsClassification();
  };

  const onReset = () => form.resetFields();

  const updateStateProjectsClassification = () => {
    setState(projectsClassification.state);
    setName(projectsClassification.name);
    setId(projectsClassification.id);
  }
  ...
  const fillFields = () => {
	...
	const sendData = async () => {
		...
		try {
			dispatch(
				updateProjectsClassificationById(id, values, async (response) => {
...
export default ProjectsClassificationUpdate;